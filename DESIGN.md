...
- **Free, complete, quiet.** No IAP, no accounts, no server. Sharing is
  a file between friends.
...
warmth, no exclamation points, no praise-speak. Lido observes; it never
cheers, scolds, or coaches.

The canon, from the app:
...
- **Errors are shrugs, not alarms.** "Couldn't read that file. It doesn't
  look like a Lido workout." — no "Error", no codes, no blame.
...
5. Could the copy be read aloud by a dry-witted lifeguard?
